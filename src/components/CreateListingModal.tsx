...
export function CreateListingModal({ isOpen, onClose, onSuccess }: CreateListingModalProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const currentCityCoords = CITY_COORDINATES[selectedCity] || { lat: 8.2415, lng: 124.2442 };
      
      const newMap = new maptilersdk.Map({
        container: mapContainer.current,
        style: maptilersdk.MapStyle.STREETS,
        center: [currentCityCoords.lng, currentCityCoords.lat],
        zoom: 14,
      ...
      const newMarker = new maptilersdk.Marker({ 
        ...
        .setLngLat([currentCityCoords.lng, currentCityCoords.lat])
        .addTo(newMap);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDescription('');
      setPrice('');
      setSelectedCity('Iligan City');
      setSelectedBarangay('Tibanga');
      setSpecificAddress('');
